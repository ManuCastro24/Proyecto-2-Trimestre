	    ...
	    */
	    ...
	    public static void matricular(Alumnos[] alumnos, Profesores[] profesores, CursoPresencial[] cursosPresenciales, CursoVirtual[] cursosVirtuales) {
	        ...
	        if (encontrado) {
	            ...
	            for (int i = 0; i < cursosPresenciales.length; i++) {
	                ...
	                if (cursoPresencial != null && cursoPresencial.getIdCursoPresencial() == idCurso) {
	                    ...
	                    for (int j = 0; j < profesores.length; j++) {
	                        ...
	                        if (profesores[j] != null && profesores[j].getDni().equals(dni)) {
	                        	
	                            cursoPresencial.agregarProfesor(profesores[j]);
	                            
	                            System.out.println("Profesor " + profesores[j].getNombre() + " matriculado exitosamente en el curso presencial " + cursoPresencial.getNomcurso());
	                    ...
	                    for (int j = 0; j < alumnos.length; j++) {
	                        ...
	                        if (alumnos[j] != null && alumnos[j].getDni().equals(dni)) {
	                        	
	                            if (cursoPresencial.agregarAlumno(alumnos[j])) {
	                            	
	                                System.out.println("Alumno " + alumnos[i].getNombre() + " matriculado exitosamente en el curso presencial " + cursoPresencial.getNomcurso());
	            ...
	            // Si el curso presencial no se encontró, vamos a comprobar si es un curso virtual
	            if (!cursoEncontrado) {
	                    ...
	                    if (cursoVirtual != null && cursoVirtual.getIdCursoVirtual() == idCurso) {
	                        ...
	                        for (int j = 0; j < profesores.length; j++) {
	                            ...
	                            if (profesores[j] != null && profesores[j].getDni().equals(dni)) {
	                                ...
	                                cursoVirtual.agregarProfesor(profesores[j]);
	                                
	                                System.out.println("Profesor " + profesores[j].getNombre() + " matriculado exitosamente en el curso virtual " + cursoVirtual.getNomcurso());
	                                
	                                return; 
	                            }
	                        }
	                        
	                        // Si el usuario no es profesor, vamos a comprobar si es alumno
	                        
	                        for (int j = 0; j < alumnos.length; j++) {
	                        	
	                            if (alumnos[j] != null && alumnos[j].getDni().equals(dni)) {
	                            	
	                                if (cursoVirtual.agregarAlumno(alumnos[j])) {
	                                	
	                                    System.out.println("Alumno " + alumnos[i].getNombre() + " matriculado exitosamente en el curso virtual " + cursoVirtual.getNomcurso());
	            ...
	            if (!cursoEncontrado) {
	                ...
	                System.out.print("¿Qué tipo de curso desea agregar con ID " + idCurso + "? (P/V): ");
	                String opcion = scanner.nextLine();
